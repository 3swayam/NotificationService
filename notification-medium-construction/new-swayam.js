...
exports.handler = async (event, context) => {
    ...
    if (params.MessageBody !== undefined && message.medium !== CONSTANT.MEDIUM_SMS_TRANSACTIONAL.NAME) {
        ...
        try {
            ...
            console.log("Updating status in dynamo from constructed  : ", response.MessageId);
            await layer.$lib.dynamoUtils.updateSuccess(message.notificationId, message.userId, response);

        } catch (err) {
            console.log("Error received for pinpoint in medium constructed  : ", message.notificationId);
